// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.13;

import "./interfaces/IInvestmentEngine.sol";
import "./interfaces/IPlanManager.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol";

/**
 * @title InvestmentEngineV2
 * @dev Enhanced InvestmentEngine with real ERC20 token integration
 */
contract InvestmentEngineV2 is IInvestmentEngine {
    // State variables
    uint256 private _depositCounter;
    uint256 private _investmentCounter;
    address public owner;
    address public planManager;
    uint256 public minimumDeposit;

    // ERC20 Integration
    address public baseToken; // Base token for deposits (e.g., USDC)

    // Storage mappings - now track actual token balances
    mapping(uint256 => UserDeposit) private _deposits;
    mapping(address => uint256[]) private _userDeposits;
    mapping(uint256 => Investment) private _investments;
    mapping(address => uint256[]) private _userInvestments;
    mapping(address => UserBalance) private _userBalances; // Still in base token units

    // New: Track actual token holdings per user per token
    mapping(address => mapping(address => uint256)) private _userTokenBalances;
    mapping(address => address[]) private _userTokens; // Tokens held by user

    // Modifiers
    modifier onlyOwner() {
        require(msg.sender == owner, "Only owner can call this function");
        _;
    }

    modifier depositExists(uint256 depositId) {
        require(
            depositId > 0 && depositId <= _depositCounter,
            "Deposit does not exist"
        );
        _;
    }

    modifier investmentExists(uint256 investmentId) {
        require(
            investmentId > 0 && investmentId <= _investmentCounter,
            "Investment does not exist"
        );
        _;
    }

    constructor(address _baseToken) {
        owner = msg.sender;
        baseToken = _baseToken;
        minimumDeposit = 10000 * 10 ** 6; // 100 USDC (6 decimals)
    }

    // MAJOR CHANGE: Deposit functions now handle real ERC20 tokens
    function depositToken(
        address token,
        uint256 amount,
        DepositType depositType
    ) external {
        require(amount >= minimumDeposit, "Amount below minimum deposit");
        require(token != address(0), "Invalid token address");

        // Transfer tokens from user to contract
        IERC20(token).transferFrom(msg.sender, address(this), amount);

        _processDeposit(msg.sender, amount, depositType);
    }

    function deposit(uint256 amount, DepositType depositType) external {
        require(amount >= minimumDeposit, "Amount below minimum deposit");

        // Transfer base tokens from user to contract
        IERC20(baseToken).transferFrom(msg.sender, address(this), amount);

        _processDeposit(msg.sender, amount, depositType);
    }

    function depositForUser(
        address user,
        uint256 amount,
        DepositType depositType
    ) external onlyOwner {
        require(user != address(0), "Invalid user address");
        require(amount >= minimumDeposit, "Amount below minimum deposit");

        _processDeposit(user, amount, depositType);
    }

    function batchDeposit(
        address[] memory users,
        uint256[] memory amounts,
        DepositType depositType
    ) external onlyOwner {
        require(users.length == amounts.length, "Arrays length mismatch");
        require(users.length > 0, "Empty arrays");

        for (uint256 i = 0; i < users.length; i++) {
            require(users[i] != address(0), "Invalid user address");
            require(
                amounts[i] >= minimumDeposit,
                "Amount below minimum deposit"
            );

            _processDeposit(users[i], amounts[i], depositType);
        }
    }

    // MAJOR CHANGE: Investment execution now involves actual token swaps
    function executeInvestment(
        uint256 investmentId
    ) external onlyOwner investmentExists(investmentId) {
        Investment storage investment = _investments[investmentId];
        require(
            investment.status == InvestmentStatus.Pending,
            "Investment not pending"
        );

        address user = investment.user;
        uint256 planId = investment.planId;
        uint256 amount = investment.totalAmount;

        // Get plan allocations
        IPlanManager planManagerContract = IPlanManager(planManager);
        IPlanManager.InvestmentPlan memory plan = planManagerContract.getPlan(
            planId
        );

        // Execute token purchases according to plan allocations
        _executePlanAllocations(user, amount, plan.allocations);

        // Update investment status
        investment.status = InvestmentStatus.Executed;
        investment.executedAmount = amount;
        investment.executedAt = block.timestamp;

        // Update user balance
        _userBalances[user].pendingInvestment -= amount;
        _userBalances[user].totalInvested += amount;

        emit InvestmentExecuted(investmentId, user, amount);
    }

    function batchExecuteInvestments(
        uint256[] memory investmentIds
    ) external onlyOwner {
        require(investmentIds.length > 0, "Empty investment IDs array");

        for (uint256 i = 0; i < investmentIds.length; i++) {
            uint256 investmentId = investmentIds[i];

            if (investmentId > 0 && investmentId <= _investmentCounter) {
                Investment storage investment = _investments[investmentId];

                if (investment.status == InvestmentStatus.Pending) {
                    address user = investment.user;
                    uint256 planId = investment.planId;
                    uint256 amount = investment.totalAmount;

                    // Get plan allocations
                    IPlanManager planManagerContract = IPlanManager(
                        planManager
                    );
                    IPlanManager.InvestmentPlan
                        memory plan = planManagerContract.getPlan(planId);

                    // Execute token purchases according to plan allocations
                    _executePlanAllocations(user, amount, plan.allocations);

                    // Update investment status
                    investment.status = InvestmentStatus.Executed;
                    investment.executedAmount = amount;
                    investment.executedAt = block.timestamp;

                    // Update user balance
                    _userBalances[user].pendingInvestment -= amount;
                    _userBalances[user].totalInvested += amount;

                    emit InvestmentExecuted(investmentId, user, amount);
                }
            }
        }
    }

    // NEW: Execute plan allocations with real token purchases
    function _executePlanAllocations(
        address user,
        uint256 totalAmount,
        IPlanManager.AssetAllocation[] memory allocations
    ) internal {
        for (uint256 i = 0; i < allocations.length; i++) {
            IPlanManager.AssetAllocation memory allocation = allocations[i];

            // Calculate token amount based on percentage
            uint256 tokenAmount = (totalAmount * allocation.targetPercentage) /
                10000;

            if (tokenAmount > 0) {
                // For now, simulate token purchase by transferring from contract reserves
                // In production, this would integrate with DEX protocols
                _purchaseToken(user, allocation.tokenAddress, tokenAmount);
            }
        }
    }

    // NEW: Token purchase logic (simplified for testing)
    function _purchaseToken(
        address user,
        address token,
        uint256 amountInBaseToken
    ) internal {
        // Simple 1:1 conversion for testing (in production, use DEX pricing)
        uint256 tokenAmount = _convertBaseToToken(token, amountInBaseToken);

        // Check if contract has enough tokens
        require(
            IERC20(token).balanceOf(address(this)) >= tokenAmount,
            "Insufficient token reserves"
        );

        // Update user token balance
        if (_userTokenBalances[user][token] == 0) {
            _userTokens[user].push(token);
        }
        _userTokenBalances[user][token] += tokenAmount;
    }

    // NEW: Convert base token amount to target token amount
    function _convertBaseToToken(
        address token,
        uint256 baseAmount
    ) internal view returns (uint256) {
        uint8 baseDecimals = IERC20Metadata(baseToken).decimals();
        uint8 tokenDecimals = IERC20Metadata(token).decimals();

        // Simple 1:1 price for testing
        // In production, get price from oracle or DEX
        if (tokenDecimals >= baseDecimals) {
            return baseAmount * (10 ** (tokenDecimals - baseDecimals));
        } else {
            return baseAmount / (10 ** (baseDecimals - tokenDecimals));
        }
    }

    // NEW: Withdraw specific tokens
    function withdrawToken(address token, uint256 amount) external {
        require(
            _userTokenBalances[msg.sender][token] >= amount,
            "Insufficient token balance"
        );

        _userTokenBalances[msg.sender][token] -= amount;
        IERC20(token).transfer(msg.sender, amount);
    }

    // NEW: Get user's token balance
    function getUserTokenBalance(
        address user,
        address token
    ) external view returns (uint256) {
        return _userTokenBalances[user][token];
    }

    // NEW: Get all tokens held by user
    function getUserTokens(
        address user
    ) external view returns (address[] memory) {
        return _userTokens[user];
    }

    // NEW: Get user's portfolio value in base token
    function getUserPortfolioValue(
        address user
    ) external view override returns (uint256) {
        UserBalance memory balance = _userBalances[user];
        uint256 baseValue = balance.totalInvested +
            balance.pendingInvestment +
            balance.availableBalance;

        // Add value of token holdings (simplified 1:1 conversion for testing)
        address[] memory tokens = _userTokens[user];
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 tokenBalance = _userTokenBalances[user][tokens[i]];
            baseValue += _convertTokenToBase(tokens[i], tokenBalance);
        }

        return baseValue;
    }

    // NEW: Convert token amount to base token value
    function _convertTokenToBase(
        address token,
        uint256 tokenAmount
    ) internal view returns (uint256) {
        uint8 baseDecimals = IERC20Metadata(baseToken).decimals();
        uint8 tokenDecimals = IERC20Metadata(token).decimals();

        // Simple 1:1 price for testing
        if (baseDecimals >= tokenDecimals) {
            return tokenAmount * (10 ** (baseDecimals - tokenDecimals));
        } else {
            return tokenAmount / (10 ** (tokenDecimals - baseDecimals));
        }
    }

    // UPDATED: Admin function to fund contract with tokens for testing
    function fundContractWithTokens(
        address token,
        uint256 amount
    ) external onlyOwner {
        IERC20(token).transferFrom(msg.sender, address(this), amount);
    }

    // Keep existing functions for backward compatibility
    function invest(
        uint256 planId,
        uint256 amount
    ) external returns (uint256 investmentId) {
        require(amount > 0, "Investment amount must be greater than 0");
        require(planManager != address(0), "Plan manager not set");
        require(
            _userBalances[msg.sender].availableBalance >= amount,
            "Insufficient balance"
        );

        // Validate that the plan exists and is active
        IPlanManager planManagerContract = IPlanManager(planManager);
        IPlanManager.InvestmentPlan memory plan = planManagerContract.getPlan(
            planId
        );
        require(plan.planId == planId, "Plan does not exist");
        require(plan.isActive, "Plan is not active");

        _investmentCounter++;
        investmentId = _investmentCounter;

        // Create investment record
        _investments[investmentId] = Investment({
            investmentId: investmentId,
            user: msg.sender,
            planId: planId,
            totalAmount: amount,
            status: InvestmentStatus.Pending,
            executedAmount: 0,
            timestamp: block.timestamp,
            executedAt: 0
        });

        // Update user balance
        _userBalances[msg.sender].availableBalance -= amount;
        _userBalances[msg.sender].pendingInvestment += amount;

        // Track user investments
        _userInvestments[msg.sender].push(investmentId);

        return investmentId;
    }

    // Keep existing view functions
    function getUserBalance(
        address user
    ) external view returns (UserBalance memory) {
        return _userBalances[user];
    }

    function getUserDeposits(
        address user
    ) external view returns (UserDeposit[] memory) {
        uint256[] memory depositIds = _userDeposits[user];
        UserDeposit[] memory deposits = new UserDeposit[](depositIds.length);

        for (uint256 i = 0; i < depositIds.length; i++) {
            deposits[i] = _deposits[depositIds[i]];
        }

        return deposits;
    }

    function getUserInvestments(
        address user
    ) external view returns (Investment[] memory) {
        uint256[] memory investmentIds = _userInvestments[user];
        Investment[] memory investments = new Investment[](
            investmentIds.length
        );

        for (uint256 i = 0; i < investmentIds.length; i++) {
            investments[i] = _investments[investmentIds[i]];
        }

        return investments;
    }

    function getInvestment(
        uint256 investmentId
    ) external view investmentExists(investmentId) returns (Investment memory) {
        return _investments[investmentId];
    }

    function getPendingInvestments(
        address user
    ) external view returns (Investment[] memory) {
        uint256[] memory investmentIds = _userInvestments[user];

        // Count pending investments
        uint256 pendingCount = 0;
        for (uint256 i = 0; i < investmentIds.length; i++) {
            if (
                _investments[investmentIds[i]].status ==
                InvestmentStatus.Pending
            ) {
                pendingCount++;
            }
        }

        // Create array of pending investments
        Investment[] memory pendingInvestments = new Investment[](pendingCount);
        uint256 index = 0;

        for (uint256 i = 0; i < investmentIds.length; i++) {
            if (
                _investments[investmentIds[i]].status ==
                InvestmentStatus.Pending
            ) {
                pendingInvestments[index] = _investments[investmentIds[i]];
                index++;
            }
        }

        return pendingInvestments;
    }

    function getTotalValueLocked() external view returns (uint256) {
        return IERC20(baseToken).balanceOf(address(this));
    }

    // Internal helper functions
    function _processDeposit(
        address user,
        uint256 amount,
        DepositType depositType
    ) internal {
        _depositCounter++;
        uint256 depositId = _depositCounter;

        // Create deposit record
        _deposits[depositId] = UserDeposit({
            user: user,
            amount: amount,
            depositType: depositType,
            timestamp: block.timestamp,
            processed: true
        });

        // Update user balance
        _userBalances[user].totalDeposited += amount;
        _userBalances[user].availableBalance += amount;

        // Track user deposits
        _userDeposits[user].push(depositId);

        emit DepositReceived(user, amount, depositType);
    }

    // Administrative Functions
    function setPlanManager(address _planManager) external onlyOwner {
        require(_planManager != address(0), "Invalid plan manager address");
        planManager = _planManager;
    }

    function setMinimumDeposit(uint256 amount) external onlyOwner {
        minimumDeposit = amount;
    }

    function setBaseToken(address _baseToken) external onlyOwner {
        require(_baseToken != address(0), "Invalid base token address");
        baseToken = _baseToken;
    }

    // NEW: Rebalancing function placeholder
    function rebalance(address user, uint256 planId) external onlyOwner {
        require(user != address(0), "Invalid user address");

        // Get current plan allocations
        IPlanManager planManagerContract = IPlanManager(planManager);
        IPlanManager.InvestmentPlan memory plan = planManagerContract.getPlan(
            planId
        );

        // TODO: Implement rebalancing logic
        // 1. Calculate current allocation percentages
        // 2. Compare with target percentages
        // 3. Execute trades to rebalance

        // For now, just emit an event
        // emit PortfolioRebalanced(user, planId);
    }
}
